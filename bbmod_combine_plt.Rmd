...
potus_bb_sim <- function(s_id, grd = g_seq){
  ...
  potus_i <- outer(grd,grd,potus, dat=diffs_i)
  potus_j <- outer(grd,grd,potus, dat=diffs_j)
  ...
  names(potus_vec)<-paste0(sort(rep(potus_labs, length(grd)^2 )),"_g_",grd_nms)
  
  c(s_id=s_id, potus_vec)
}

# function to reshape combined output of potus_bb_sim
...
doBatch <- function(all_sim_ids, batchsize){
  ...
  for (i in 1:batches){
    ...
    # potus_bb_sim for batch - prob of technical success
    assign(paste0("bb_potus_dat0_",i),
             data.frame(do.call(rbind, lapply(sims_in_batch, potus_bb_sim))),
             envir=.GlobalEnv)
...
bb_potus_vec <- ls()[grep("bb_potus_dat0_",ls())]
bb_potus_dat0 <- do.call(rbind, sapply(bb_potus_vec, get,simplify = FALSE) )
bb_potus_dat1 <- reshaper5(bb_potus_dat0)
bb_potus_dat <- merge(bb_sim_array_ord_run, bb_potus_dat1, by.x="sim_id",by.y="s_id")
rm(list=bb_potus_vec)
rm(bb_potus_dat0,bb_potus_vec)
```
...
png(file.path(figdir,"bb_ciwidths_sub_plt.png"), width=1800, height=600, pointsize=5)
bb_ciwidths_sub_plt # <-- output for poster?
dev.off()
```

```{r bb-ci-widths-overall-plt}
...
ggplot(data=subset(bb_pp_dat_cln,rho_2==0.35 & n==200 & model=="copula"), 
  ...
  geom_line(alpha=0.5) + facet_grid(delta_e~delta_s,labeller = "label_both") + theme(legend.position = "none", axis.text.x= element_text(size=10))
...
ggplot(data=subset(bb_pp_dat_summ, rho_2==0.35 & n==400 & outcome=="efficacy"), 
       aes(x=g, y=mean_pp_gt_g, color=model, group=model)) + 
  geom_line(alpha=0.5) + facet_grid(delta_e~delta_s,labeller = "label_both")
...
bb_potus_dat_cln <- bb_potus_dat %>% mutate(delta_e=p_e2-p_e1, delta_s=p_s2-p_s1, 
                      model = factor(mod,c("i","j"), c("independent","copula")) ) %>%
                  select(-c(dat_seed,samp_seed,mod,p_e2,p_s2)) 

bb_potus_dat_summ <- bb_potus_dat_cln %>% 
  arrange(scn_id, model, g_e, g_s) %>%
  group_by(scn_id, model, g_e, g_s) %>%
  summarise(mn_potus = mean(potus_g), n=mean(n), rho_2=mean(rho_2), delta_e=mean(delta_e),delta_s=mean(delta_s))
...
zvec_i <- unlist(subset(bb_potus_dat_summ, scn_id==id & model=="independent", select=c("mn_potus")))

zvec_j <- unlist(subset(bb_potus_dat_summ, scn_id==id & model=="copula", select=c("mn_potus")))
...
if (num==1){
filled.contour(g_seq, g_seq, z=t(zmat_i), col = potus_col, 
...
filled.contour(g_seq, g_seq, z=t(zmat_j), col = potus_col, 
               key.title=title(main = "Posterior \nProbability (%)", cex.main=0.7), 
...
png(file.path(figdir,"bb_potus_r035_plt1.png"), width=900, height=600, pointsize=20)
plot_potus(69,1) # <--
dev.off()

png(file.path(figdir,"bb_potus_r035_plt2.png"), width=900, height=600, pointsize=20)
plot_potus(69,2) # <--
dev.off()

png(file.path(figdir,"bb_potus_r035_plt3.png"), width=900, height=600, pointsize=20)
plot_potus(69,3) # <--
dev.off()
