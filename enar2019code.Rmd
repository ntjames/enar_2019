...
1) binary - binary (Meester example)
2) continuous - continuous
3) continuous - binary (ex. 1 from Costa)
4) ordinal rather than binary??
...
try multivariate probit model; should match above, see STAN user's guide v2.18 sec 2.15 pdf p 42; not really working so far
...
stan_trace(bb_fit3, pars=c("beta"))
stan_trace(bb_fit3, pars=c("p_e"))
stan_trace(bb_fit3, pars=c("p_s"))
...
### Max Likelihood from Meester and MacKay
...
try to use coded log-likelihood function rather than hardcoding
```{r br-d, cache=TRUE}
# joint model
mod_code2 <- "
functions {
